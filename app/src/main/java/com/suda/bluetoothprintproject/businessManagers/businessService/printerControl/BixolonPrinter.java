...
public class BixolonPrinter implements OutputCompleteListener, ErrorListener, StatusUpdateListener
{
	//, DirectIOListener, DataListener {
	// ------------------- alignment ------------------- //
	...
	private PrinterLogCallback mPrinterLogCallback = null;
	...
	public BixolonPrinter(PrinterLogCallback mPrinterLogCallback) {
		this.mPrinterLogCallback = mPrinterLogCallback;
		
		// 這裡應該是監聽 第三方套件
		posPrinter = new POSPrinter(this.mPrinterLogCallback.getCurrentContext());
		...
		bxlConfigLoader = new BXLConfigLoader(this.mPrinterLogCallback.getCurrentContext());
		try {
	...
	// 確認有無印表機連線
	public boolean CheckIsConnPrint() {
		boolean result = false;
		try {
			result = posPrinter.getDeviceEnabled();
		}
	...
	// TODO: 印表機連接 (若重新連線一台已經連線過的設備, 則會斷開)
	...
	public boolean printerClose() {
		try {
			if(posPrinter.getClaimed()) {
				...
				posPrinter.close();
				this.mPrinterLogCallback.printerLogCallback("StatusUpdate : Power off");
			}
		}
		catch(JposException e) {
			this.mPrinterLogCallback.printerLogCallback("ERROR: When Power off Error");
			e.printStackTrace();
		}
		...
		try {
			for(Object entry : bxlConfigLoader.getEntries()) {
				...
				if(jposEntry.getLogicalName().equals(logicalName)) {
					bxlConfigLoader.removeEntry(jposEntry.getLogicalName()); // 移除對應型號的設定檔
				}
			}
	...
	// TODO: 這裡是列印文字的方法, 或許可以從使用方法試 列印指令
	// 目前不使用這個方法列印
	...
	////			}
	////		}
	//		((BluetoothActivity)context).setDeviceLog("DirectIOEvent : " + getBatterStatusString(directIOEvent.getData()));
	//	}
	
	@Override
	public void errorOccurred(ErrorEvent errorEvent) {
		this.mPrinterLogCallback.printerLogCallback("Error : " + errorEvent);
	}
	
	//TODO: 若印表機使用非同步, 輸出完畢的監聽
	@Override
	public void outputCompleteOccurred(OutputCompleteEvent outputCompleteEvent) {
		...
		//			e.printStackTrace();
		//		}
		//		((BluetoothActivity)context).setDeviceLog(getSUEMessage(JposConst.JPOS_SUE_POWER_OFF_OFFLINE));
		this.mPrinterLogCallback.printerLogCallback("outputComplete : " + outputCompleteEvent.getOutputID());
	}
	
	@Override
	public void statusUpdateOccurred(StatusUpdateEvent statusUpdateEvent) {
		this.mPrinterLogCallback.printerLogCallback(getSUEMessage(statusUpdateEvent.getStatus()));
	}
	...
	////		}
	//
	//		((BluetoothActivity)context).setDeviceLog("DataEvent : " + DataEvent.incrSequenceNumber());
	//	}
